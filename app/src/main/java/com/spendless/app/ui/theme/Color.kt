...
object AppColors{
    ...
    val PrimaryOpacity16 = Color(0xFF5A00C8).copy(alpha = 0.8f)
...
object AppIconButtonColors {
    @Composable
    fun filled(): IconButtonColors {
        return IconButtonDefaults.iconButtonColors(
            containerColor = MaterialTheme.colorScheme.primary, // Основной цвет кнопки
            contentColor = MaterialTheme.colorScheme.onPrimary, // Цвет иконки
            disabledContainerColor = MaterialTheme.colorScheme.surfaceVariant, // Отключённый фон
            disabledContentColor = MaterialTheme.colorScheme.onSurfaceVariant // Отключённая иконка
        )
    }

    @Composable
    fun standard(): IconButtonColors {
        return IconButtonDefaults.iconButtonColors(
            containerColor = Color.Transparent, // Без фона
            contentColor = MaterialTheme.colorScheme.onSurface, // Цвет иконки
            disabledContentColor = MaterialTheme.colorScheme.onSurfaceVariant // Отключённая иконка
        )
    }

    @Composable
    fun error(): IconButtonColors {
        return IconButtonDefaults.iconButtonColors(
            containerColor = MaterialTheme.colorScheme.errorContainer, // Ошибочный фон
            contentColor = MaterialTheme.colorScheme.error, // Ошибочный цвет иконки
            disabledContainerColor = MaterialTheme.colorScheme.surfaceVariant,
            disabledContentColor = MaterialTheme.colorScheme.onSurfaceVariant
        )
    }
